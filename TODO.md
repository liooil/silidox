...
- [x] 保留无障碍地表横向移动和掉头，并把开局地点改为森林。
- [x] 让三个林地树木点慢速再生，作为长期伐木补给。
- [x] 增加独立木材资源，并允许玩家在建设与燃烧供能之间取舍。
- [x] 以六处周期再生掉落树枝作为第一阶段木材来源，并建立开局燃料安全模型。
- [x] 将拾取树枝后的第二个小游戏确定为挖矿，并用木材装配简易掘进头。
- [x] 在九格森林末端建立固定入口，把地下升级为最大十二层的二叉矿路。
- [x] 保存 `L/R` 矿路节点与左右未完成进度，按深度提高岩层硬度与结构料产出。
- [x] 用路径确定性矿层替代有限二维矿点，并加入左右输出的采掘控制器与梯形图设备域。
- [x] 矿井视图只显示当前位置及其已挖子树，进入深层后逐步缩放，并由视图整体滚动。
- [x] 为心跳、移动和拾取提供不要求编程的低效预设控制器。
- [x] 让设备梯形图降低扫描负载与能源消耗。
- [x] 增加维修台、木气化炉和频谱传感器建设任务。
...
- [x] 增加地下采掘与矿石/部件资源，建立持续原料来源。
- [x] 搭建部件加工台，把矿石加工成标准部件。
- [ ] 扩展工坊到完整 40–60 分钟物理生存章节（运输与灵性材料链）。
...
设计基线见 [world/inner-landscape.md](world/inner-landscape.md)。本节按照“残阵证据之后，无根到筑基”的实现依赖排序；它不会改变当前前期物理开局的范围。
