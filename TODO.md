...
- [x] 让三个林地采集点的树木定时再生，支持反复伐木。
- [x] 增加独立木材资源，并允许玩家在建设与燃烧供能之间取舍。
- [x] 以木气化炉替代免费持续供能，能源偏低时自动消耗木材并保留应急燃料。
- [x] 将伐木后的第二个小游戏确定为挖矿，并用木材装配简易掘进头。
- [x] 将一维森林升级为地表横向位置与向下深度组成的二维剖面。
- [x] 保存地下格、竖井和未完成掘进进度，按深度提高岩层硬度。
- [x] 用固定矿层替代地表矿脉刷新，并加入采掘控制器与梯形图设备域。
