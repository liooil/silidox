...
- 推进核心衰减、停机恢复、木气化供能和建设计时
  - 执行地表移动、掉头、伐木、燃烧木材、左右分支掘进、向上和控制器动作
- 运行预设控制与设备梯形图输出
...
- 开局先手动心跳，再解锁环境和自动化。
- 核心归零是可恢复停机，不清空进度。
- 主角在森林中复苏；开局只显示无障碍地表横轴，先在六处会较快再生的掉落树枝中拾取初期木材，装配掘进头后在固定入口展开二叉矿路。
- 成熟树木只慢速再生，伐木是后续长期补给；木材既用于建设，也能手动或通过木气化炉转化为能源。
- 挖矿是拾取树枝后的第二个手动小游戏。每个节点可向左下或右下掘进，岩层需要多次操作才会击穿；矿路与两侧未完成进度永久保存，深层硬度和产出更高。
- 地下节点使用 `L/R` 路径确定矿物，不依赖随机地图。视图只显示当前节点及其已挖掘子树，进入深层后逐步缩放；不加入自由洞穴寻路或坍塌。
...
- 装配掘进头后展开二叉矿路，验证左右掘进、父节点往返、子树可见性、深层缩放和地下残阵
- 控制器安装、预设运行与梯形图接管
- 工坊建设进度和残阵三次采样
- 梯形图点击、拖放、重排和整体滚动
- 桌面与移动端无文字或控件重叠
